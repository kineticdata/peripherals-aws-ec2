...
class AwsEc2DeleteRouteTableV1
  ...
  def execute()
    puts "Attempting to delete route table '#{@parameters['route_table_id']}'" if @enable_debug_logging
    resp = @ec2.delete_route_table(
      dry_run: @parameters['dry_run'],
      route_table_id: @parameters['route_table_id']
    )

    puts "Route table '#{@parameters['route_table_id']}' deleted successfully." if @enable_debug_logging
