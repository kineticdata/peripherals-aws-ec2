...
class AwsEc2DeleteNetworkInterfaceV1
  ...
  def execute()
    puts "Attempting to delete the network interface '#{@parameters['network_interface_id']}'" if @enable_debug_logging
    resp = @ec2.delete_network_interface(
      dry_run: @parameters['dry_run'],
      network_interface_id: @parameters['network_interface_id']
    )

    puts "Network Interface #{@parameters['network_interface_id']} deleted successfully." if @enable_debug_logging
    <<-RESULTS
    <results />
    RESULTS
  end
