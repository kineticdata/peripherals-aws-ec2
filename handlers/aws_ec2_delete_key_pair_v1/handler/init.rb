...
class AwsEc2DeleteKeyPairV1
  ...
  def execute()
    puts "Attempting to delete the key pair '#{@parameters['key_name']}'" if @enable_debug_logging
    resp = @ec2.delete_key_pair(
      dry_run: @parameters['dry_run'],
      key_name: @parameters['key_name']
    )

    puts "Key name '#{@parameters['key_name']}' successfully deleted from EC2." if @enable_debug_logging
    return <<-RESULTS
    <results />
    RESULTS
  end
