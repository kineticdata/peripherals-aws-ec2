...
class AwsEc2CreateRouteV1
  ...
  def execute()
    puts "Validating the parameter inputs" if @enable_debug_logging
    target_count = 0
    ['gateway_id','instance_id','network_interface_id','vpc_peering_connection_id'].each do |field|
      target_count += 1 if !@parameters[field].to_s.strip.empty?
    end

    raise "Exactly one of the following fields must be supplied: "+
      "'Gateway ID', 'NAT Instance ID', 'VPC Peering Connection ID', 'Network Interface ID'" if target_count != 1

    puts "Attempting to create the route" if @enable_debug_logging
    ...
    puts "Successfully added route to route table #{@parameters['route_table_id']}" if @enable_debug_logging
