...
class AwsEc2DeleteSubnetV1
  ...
  def execute()
    puts "Attempting to delete the subnet '#{@parameters['subnet_id']}'" if @enable_debug_logging
    resp = @ec2.delete_subnet(
      dry_run: @parameters['dry_run'],
      subnet_id: @parameters['subnet_id']
    )

    puts "Subnet #{@parameters['subnet_id']} deleted successfully." if @enable_debug_logging
    <<-RESULTS
    <results />
    RESULTS
  end
