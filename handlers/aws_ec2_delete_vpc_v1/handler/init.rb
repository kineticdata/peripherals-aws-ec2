...
class AwsEc2DeleteVpcV1
  ...
  def execute()
    puts "Attempting to delete VPC '#{@parameters['vpc_id']}'" if @enable_debug_logging
    resp = @ec2.delete_vpc(
      dry_run: @parameters['dry_run'],
      vpc_id: @parameters['vpc_id']
    )

    puts "VPC #{@parameters['vpc_id']} deleted successfully." if @enable_debug_logging
    <<-RESULTS
    <results />
    RESULTS
  end
