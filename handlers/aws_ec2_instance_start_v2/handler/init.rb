...
class AwsEc2InstanceStartV2
  ...
  def execute()
    puts "Attempting to start the ec2 instance '#{@parameters['instance_id']}'" if @enable_debug_logging
    resp = @ec2.start_instances(
      dry_run: @parameters['dry_run'],
      instance_ids: [@parameters['instance_id']]
    )
    puts "EC2 Instance successfully started" if @enable_debug_logging

    # We will set the returned values into intermediate variables to improve
    # readability of the results
    previous_status = resp.starting_instances[0].previous_state.name
    current_status = resp.starting_instances[0].current_state.name
