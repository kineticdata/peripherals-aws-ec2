...
class AwsEc2AssociateRouteTableV1
  ...
  def execute()
    puts "Attempting to associate the Subnet '#{@parameters['subnet_id']}' with the Route Table '#{@parameters['route_table_id']}'" if @enable_debug_logging
    ...
    puts "Association successful with a returned id of '#{resp.association_id}'" if @enable_debug_logging
    return <<-RESULTS
    <results>
      <result name="Association Id">#{escape(resp.association_id)}</result>
    </results>
    RESULTS
  end
