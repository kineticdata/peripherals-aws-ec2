...
class AwsEc2CreateSecurityGroupV1
  ...
  def execute()
    ...
    puts "Group ID '#{resp.group_id} was successfully created." if @enable_debug_logging

    <<-RESULTS
    <results>
      <result name="Group ID">#{escape(resp.group_id)}</result>
    </results>
    RESULTS
  end
